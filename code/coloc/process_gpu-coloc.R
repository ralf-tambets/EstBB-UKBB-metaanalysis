...
#counted_signals = readr::read_tsv("data/big_data/annotated_gpu-coloc_results/meta_EUR_big_coloc_151025.tsv")


#Find clusters that colocalise with both Albumin and GlycA
alb_glyca_clusters = dplyr::filter(counted_signals, metabolite %in% c("Albumin","GlycA")) %>% 
  dplyr::select(metabolite, cluster) %>% dplyr::arrange(cluster) %>% 
  dplyr::distinct() %>% dplyr::group_by(cluster) %>% 
  dplyr::mutate(met_count = n()) %>% 
  dplyr::filter(met_count > 1)
